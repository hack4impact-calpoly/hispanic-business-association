...
// Website URL validation matching backend logic
...
// ZIP code validation matching backend requirements
...
export default function EditBusinessInfo({ onClose, onSubmitSuccess }: EditBusinessInfoProps) {
  ...
  const handleSubmit = async () => {
    ...
    // Website validation - optional but format checked if provided
    if (formData.website.trim() && !isValidWebsite(formData.website.trim())) {
      errors.website = t("websiteFormatInvalid");
    }
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-5">
        <form className="space-y-4 pb-20">
          ...
          <div>
            ...
            />
            {validationErrors.website && <p className="text-red-600 text-sm mt-1">{validationErrors.website}</p>}
          </div>
